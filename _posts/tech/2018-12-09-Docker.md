...
其中仓库名、标签在之前的基础概念章节已经介绍过 了。镜像 ID 则是镜像的唯一标识，一个镜像可以对 应多个标签。因此，在上面的例子中，我们可以看到 ubuntu:16.04 和 ubuntu:latest 拥有相同的 ID，因 为它们对应的是同一个镜像。
...
这里的-a是--attach参数的缩写，请多多利用 docker start –help确认
...
docker 还提供了docker atach命令，但是退出时会导 致容器停止，不推荐使用。

镜像中使用了不同的Shelll，使用前需要查询下基础镜 像名，一般alpine镜像用的是ash
...
Compose 项目是 Docker 官方的开源项目，负责实 现对 Docker 容器的快速编排，提升本地开发和测试 效率。
...
Compose 支持 Linux、macOS、Windows 10 三大平 台。
